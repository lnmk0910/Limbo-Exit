// UpgradeScreen.cs — Màn hình nâng cấp vĩnh viễn: tốc độ, la bàn, giảm giá, tầm phát hiện
using UnityEngine;
using TMPro;
...
public class UpgradeScreen : MonoBehaviour
{
    ...
    // Lang nghe phim tat trong man hinh nang cap
    void Update()
    {
        if (panelUpgrade == null || !panelUpgrade.activeSelf) return;
    ...
    // Mo man hinh nang cap va cap nhat UI
    ...
    // Hien thi chi tiet cap do va gia
    void CapNhatUI()
    {
        PlayerData data = SaveSystem.LoadGame();
        if (txtManhHon != null) txtManhHon.text = $"Mảnh Hồn: {data.soManhHon} MH";

        if (txtCapTocDo != null) txtCapTocDo.text = CapString(data.capTocDo, capToiDaTocDo, "+0.5 tốc độ");
        if (txtGiaTocDo != null) txtGiaTocDo.text = data.capTocDo >= capToiDaTocDo ? "ĐÃ TỐI ĐA" : $"{giaTocDo} MH";

        if (txtCapLaBan != null) txtCapLaBan.text = CapString(data.capLaBan, capToiDaLaBan, "+1s la bàn");
        if (txtGiaLaBan != null) txtGiaLaBan.text = data.capLaBan >= capToiDaLaBan ? "ĐÃ TỐI ĐA" : $"{giaLaBan} MH";

        if (txtCapGiaShop != null) txtCapGiaShop.text = CapString(data.capGiamGiaShop, capToiDaGiamShop, "-1 giá shop");
        if (txtGiaGiaShop != null) txtGiaGiaShop.text = data.capGiamGiaShop >= capToiDaGiamShop ? "ĐÃ TỐI ĐA" : $"{giaGiamShop} MH";

        if (txtCapTamPH != null) txtCapTamPH.text = CapString(data.capTamPhatHien, capToiDaTamPH, "-1 tầm quái");
        if (txtGiaTamPH != null) txtGiaTamPH.text = data.capTamPhatHien >= capToiDaTamPH ? "ĐÃ TỐI ĐA" : $"{giaTamPhatHien} MH";
    }

    string CapString(int hienTai, int toiDa, string moTa) =>
        $"{moTa}\n[{new string('*', hienTai)}{new string('-', toiDa - hienTai)}] {hienTai}/{toiDa}";

    // Xu ly nang cap theo loai (tocdo/laban/shop/tamph)
    void ThucHienNangCap(string loai)
    {
        ...
        switch (loai)
        {
            case "tocdo": cap = data.capTocDo;      toiDa = capToiDaTocDo;    gia = giaTocDo;      break;
            case "laban": cap = data.capLaBan;       toiDa = capToiDaLaBan;    gia = giaLaBan;      break;
            case "shop":  cap = data.capGiamGiaShop; toiDa = capToiDaGiamShop; gia = giaGiamShop;   break;
            case "tamph": cap = data.capTamPhatHien; toiDa = capToiDaTamPH;    gia = giaTamPhatHien; break;
        }

        if (cap >= toiDa)        { AudioManager.PhatKhongDuTien(); return; }
        if (data.soManhHon < gia) { AudioManager.PhatKhongDuTien(); return; }
    ...
    // Dong panel nang cap va quay ve HUB
    ...
    // Dong panel va tiep tuc choi (vao scene)
